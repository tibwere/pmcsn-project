...
È immediato osservare che per rispettare i requisiti di qualità stabiliti nel capitolo \ref{chp:obiettivi}, mostrati nei grafici tramite linee rosse tratteggiate, il numero minimo di serventi è $M^* = 4$. Infatti, dalla figura \ref{fig:esperimenti-simulazione-1d} si evince che con $M = 3$ non è possibile rispettare il QoS (evidenziato in {\color{red}rosso} in tabella \ref{table:esperimenti-simulazione-1a}).

È opportuno notare che le ampiezze delle realizzazioni degli intervalli di confidenza nel caso di \sr{} sono inevitabilmente maggiori rispetto ai casi di \uo{} e \pp{} per via dell'inferiore frequenza di arrivo e quindi ridotta quantità di dati utilizzati per il calcolo delle stime.
