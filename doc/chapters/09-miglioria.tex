...
\section{Verifica}
Per la verifica, come effettuato anche nel capitolo \ref{chp:verifica}, è stata adottata la funzione \texttt{print\_update()}, modificata al fine di stampare a video anche il numero di ticket \textsl{BancoPosta}/\sr{} serviti consecutivamente, per poter controllare che tutte le modifiche introdotte con la miglioria operassero come atteso.
...
\caption{Incremento del counter di ticket \textsl{BancoPosta} serviti consecutivamente}    
...
\caption{Incremento del counter di ticket \sr{} serviti consecutivamente}    
\label{fig:miglioria-verifica-1b}
\end{subfigure}
\caption{Verifica del corretto incremento del counter}
...
\end{subfigure}
\caption{Verifica del corretto scheduling da parte del servente dedicato in assenza di ticket \sr{}}
