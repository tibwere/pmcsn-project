...
L'orologio di sistema adottato nel simulatore è contenuto all'interno di una struttura dati, riportata nello snippet \ref{lst:modello-computazionale-2}, che memorizza informazioni relative al tempo di simulazione. In particolare:
\begin{itemize}
\item \texttt{{\color{code_purple}double} next}: istante di occorenza del successivo evento nel sistema.
\item \texttt{{\color{code_purple}double} last}: istante di fine simulazione.
...
\end{itemize}
Per il caso di studio in analisi, i valori di \texttt{START}, \texttt{STOP} ed \texttt{INFTY} sono rispettivamente \texttt{0}, \texttt{???} e \texttt{100 $\cdot$ STOP}.
...
La lista degli eventi, a livello implementativo, è gestita mediante la struttura dati riportata nello snippet \ref{lst:modello-computazionale-3}.
